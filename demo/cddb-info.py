...
def main():
    ...
    print("Getting disc id in CDDB format...", end="")

    disc_id = DiscID.disc_id(cdrom)

    print("Disc ID: %08lx Num tracks: %d" % (disc_id[0], disc_id[1]))
    print("Querying CDDB for info on disc...", end="")
    ...
    if query_stat == 200:
        ...
        if read_stat == 210:
            ...
            for i in range(0, disc_id[1]):
                print("Track %.02d: %s" % (i + 1, read_info["TTITLE" + repr(i)]))
        else:
